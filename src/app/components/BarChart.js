...
const GoalProgressBarChart = ({ goalData }) => {
  ...
  useEffect(() => {
    ...
    const firstDate = orderedDataset.length > 0 ? orderedDataset[0].date : null;
    ...
    if (firstDate) {
      ...
      if (pastSevenDaysData.length === 7) {
        ...
        const startIndex = orderedDataset.findIndex((d) => d.date >= firstDate);
    ...
    svg
      ...
      .call(d3.axisBottom(xScale).tickFormat((d, i) => filteredDataset[i].day))
      .selectAll("line")
      .remove();
