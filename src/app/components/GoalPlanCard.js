...
const GoalPlanCard = ({ goalPlan, onClick }) => {
  const creatorName = goalPlan.creator.username || "Anonymous";

  return (
    <div
      onClick={onClick}
      className="cursor-pointer bg-white shadow-md rounded-lg p-4 flex flex-col justify-items-start gap-3"
    >
      <div className="flex ">
        {goalPlan.image_url ? (
          <img
            alt="Goal Plan image"
            src={goalPlan.image_url}
